...
void node_bcrypt(const char* key, size_t key_len, const char* salt, char* encrypted) {
    ...
    if (minor <= 'a') key_len = (u_int8_t) (key_len + (minor >= 'a' ? 1 : 0));
    else {
    ...
    j = 0;
    for (i = 0; i < BCRYPT_BLOCKS; i++) cdata[i] = Blowfish_stream2word(ciphertext, 4 * BCRYPT_BLOCKS, &j);

    /* Now do the encryption */
    for (k = 0; k < 64; k++) blf_enc(&state, cdata, BCRYPT_BLOCKS / 2);
